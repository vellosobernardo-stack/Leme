...
// Pro: exibe 3 períodos (hoje/mês/trimestre) com persistência no banco
// Free: exibe apenas "O que fazer hoje" — só título + tags, sem checkbox, sem descrição
...
interface PlanoAcaoSectionProps {
  ...
  analiseId?: string;
  isPro?: boolean;
  isFree?: boolean; // NOVO — modo Free: oculta descrição, resultado e checkbox
}

export default function PlanoAcaoSection({ plano, analiseId, isPro = false, isFree = false }: PlanoAcaoSectionProps) {
  ...
  useEffect(() => {
    if (!analiseId || isFree) return; // Free não carrega progresso
    ...
    carregar();
  }, [analiseId, isPro, isFree]);

  // Salva no localStorage quando mudar (Free legado only — isFree novo não salva)
  useEffect(() => {
    if (!analiseId || isPro || isFree || !carregouInicial.current) return;
  ...
  }, [marcados, analiseId, isPro, isFree]);

  const toggleItem = async (periodo: '30' | '60' | '90', index: number) => {
    if (isFree) return; // Free não interage
  ...
  const periodos = (isPro && !isFree) ? todosPeriodos : todosPeriodos.slice(0, 1);

  return (
    <div className={`grid grid-cols-1 ${!isFree ? 'md:grid-cols-3' : ''} gap-6`}>
        ...
        return (
          ...
          >
            ...
            <div className="flex items-center gap-3 mb-2">
              ...
              {/* Contador de progresso — oculto no Free */}
              {!isFree && (
            ...
            {/* Barra de progresso — oculta no Free */}
            {!isFree && (
            ...
            {/* Espaço equivalente quando sem barra de progresso */}
            ...
            <div className="space-y-4">
              {periodo.acoes.map((acao, index) => {
                ...
                // Free: div estática não-clicável
                // Pro: button clicável
                const CardWrapper = isFree ? 'div' : 'button';
                const cardProps = isFree
                  ? { className: `w-full text-left p-4 rounded-lg ${periodo.lightColor}` }
                  : {
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1">
                        ...
                        <h4 className={`font-semibold mb-2 transition-all ${
                          !isFree && isChecked ? 'text-gray-500 line-through' : 'text-gray-900'
                        }`}>
                          {acao.titulo}
                        </h4>
                          ...
                          <div className="flex flex-wrap gap-2 mb-3">
                            {(acao as any).tempo_estimado && (
                              <span className={`inline-flex items-center gap-1 text-xs px-2 py-1 rounded-full ${
                                !isFree && isChecked ? 'bg-gray-100 text-gray-400' : 'bg-gray-100 text-gray-600'
                              }`}>
                            ...
                            {(acao as any).dificuldade && (
                              <span className={`inline-flex items-center gap-1 text-xs px-2 py-1 rounded-full ${
                                !isFree && isChecked ? 'bg-gray-100 text-gray-400' :
                        ...
                        {/* Resultado esperado — oculto no Free */}
                        {!isFree && (
